...
class PhotosHelperTests(unittest.TestCase):
    ...
    def test_photos_survive_db_roundtrip(self):
        self.session = make_session()
        draft = make_draft(self.session, photos=photos_to_json(['p1', 'p2']), image='p1')
        self.session.expire_all()
...
class PublishTests(unittest.TestCase):
    ...
    def test_double_publish_blocked(self):
        ...
        self.assertEqual(get_approval(self.session, self.draft.id).status, STATUS_PUBLISHED)
...
class StubBot:
    ...
    async def send_message(self, chat_id, text, parse_mode=None, reply_markup=None):
        self.calls.append({'type': 'message', 'chat_id': chat_id, 'text': text,
                           'parse_mode': parse_mode, 'reply_markup': reply_markup})

    async def send_photo(self, chat_id, photo, caption=None, parse_mode=None, reply_markup=None):
        self.calls.append({'type': 'photo', 'chat_id': chat_id, 'photo': photo,
...
class SendPostTests(unittest.TestCase):
    ...
    def send(self, photos, post=None, extra_lines='', reply_markup=None, heading=None):
        ...
        asyncio.run(send_post(
            bot, -100500, post or self.POST, photos,
        ...
        try:
            ...
            with engine.connect() as connection:
                row = connection.execute(text(
                    "SELECT title, image, photos FROM drafts WHERE id = 1"
                )).fetchone()
