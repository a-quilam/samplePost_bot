...
class DecideTests(unittest.TestCase):
    ...
    def test_forbidden_for_other_user(self):
        ...
        self.assertEqual(
            get_approval(self.session, self.draft.id).status, STATUS_ASSIGNED
        )
...
class DraftToPostDataTests(unittest.TestCase):
    def test_keys_match_summary_format(self):
        ...
        try:
            ...
            self.assertIsNone(
                data["date"]
            )  # незаполненные поля — None, сводка покажет «Не указано»
        finally:
...
class HandlerConsistencyTests(unittest.TestCase):
    ...
    def test_approval_callbacks_not_clash_with_conversation(self):
        ...
        self.assertTrue(
            conv_patterns, "в ConversationHandler нет CallbackQueryHandler?"
        )
    ...
    def test_conversation_callbacks_not_clash_with_approval(self):
        ...
        """
        approval_patterns = [
            h.pattern for h in approval_handlers() if h.pattern is not None
        ]
